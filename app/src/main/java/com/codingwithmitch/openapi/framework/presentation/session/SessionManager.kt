package com.codingwithmitch.openapi.framework.presentation.session
...
@Inject
constructor(
    private val checkPreviousAuthUserUseCase: CheckPreviousAuthUserUseCase,
    ...
    init {
        ...
        sessionScope.launch {
            appDataStoreSource.readValue(DataStoreKeys.PREVIOUS_AUTH_USER)?.let { email ->
    ...
    private fun checkPreviousAuthUser(email: String){
        state.value?.let { state ->
            checkPreviousAuthUserUseCase.checkPreviousAuthUser(email).onEach { dataState ->
    ...
    private fun logout(){
        state.value?.let { state ->
            logoutUseCase.logout().onEach { dataState ->
                this.state.value = state.copy(isLoading = dataState.isLoading)
    ...
    private fun clearAuthUser() {
        sessionScope.launch {
            appDataStoreSource.setValue(DataStoreKeys.PREVIOUS_AUTH_USER, "")
        }
    }
